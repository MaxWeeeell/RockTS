...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        ...
        # forward
        pred, ori, masked, rec, ot, prob, mask, cost = self.model_forward()
        epsilon = 1e-9
    
        kl_divergence = prob * torch.log(prob / self.r + epsilon) + (1 - prob) * torch.log((1 - prob) / (1 - self.r) + epsilon)
        entropy_loss = torch.mean(kl_divergence)
        connect_loss = self.loss_func(mask.reshape(mask.shape[0],-1)[:,1:] , mask.reshape(mask.shape[0],-1)[:,:-1])
        bs, num_patch, n_vars, patch_len = ori.shape

        loss_kl = torch.nn.functional.kl_div(ori.transpose(1,2).reshape(bs, n_vars, -1).log_softmax(dim=-1) +epsilon, rec.transpose(1,2).reshape(bs, n_vars, -1).softmax(dim=-1)+epsilon).mean()
        loss = self.loss_func(pred, self.yb) + loss_kl + self.loss_func(ot, ori) + entropy_loss + connect_loss + self.cost_lambda*cost
    ...
    def model_forward(self):
        self('before_forward')
        self.pred, self.ori, self.masked, self.rec, self.ot, self.prob, self.mask, self.cost= self.model(self.xb)
        self('after_forward')
        return self.pred, self.ori, self.masked, self.rec, self.ot, self.prob, self.mask, self.cost
    ...
    def test_step(self, batch):
        ...
        self.xb, self.yb = batch
        # forward
        pred, _,_,_,_,_,_,_= self.model_forward()
        return pred, self.yb
    ...
    def head_first_train(self, n_epochs, base_lr=None, freeze_epochs=1, pct_start=0.3):
        """
