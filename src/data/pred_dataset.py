...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        if '_anom' in self.data_path:
            df_raw_wo_anom = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path.replace('_anom', '', 1)))
        ...
        if self.features == 'M' or self.features == 'MS':
            ...
            if '_anom' in self.data_path:
                cols_data_wo_anom = df_raw_wo_anom.columns[1:]
                df_data_wo_anom = df_raw_wo_anom[cols_data_wo_anom]
        ...
        if self.scale:
            ...
            if '_anom' in self.data_path:
                train_data = df_data_wo_anom[border1s[0]:border2s[0]]
                self.scaler.fit(train_data.values)
                data = self.scaler.transform(df_data.values)
                data_wo_anom = self.scaler.transform(df_data_wo_anom.values)
            # else:
            #     train_data = df_data[border1s[0]:border2s[0]]
            #     self.scaler.fit(train_data.values)
            #     data = self.scaler.transform(df_data.values)
        ...
        # if '_anom' in self.data_path and self.scaler =='test':
        if '_anom' in self.data_path:
            self.data_y = data_wo_anom[border1:border2]
